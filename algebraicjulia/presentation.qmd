...
- Finally, for the action on morphisms, given a cospan $I \rightarrow J \leftarrow I'$ and a resource sharer $(S, \mathrm{expose} \colon I \to S, v \colon \R^S \to \R^S)$, we make a new resource sharer $(S', \mathrm{expose}', v')$ on $I'$ by taking
...
Via the pushout, any two state variables in $S$ connected to the same junction in $J$ by a port in the interface $I$ are merged.
